// lib/billInvoice.ts
// Professional bill collection invoice generator
...
export interface BillInvoiceData {
  ...
  // Payment breakdown
  collectionMode: "monthly" | "package" | "due_only";

  // Old dues being paid
  oldDueAmount: number;
  oldDueDescription?: string;

  // Cycle charge (monthly renewal or package)
  cycleCharge: number;
...
export function openBillInvoice(data: BillInvoiceData): void {
  ...
  const modeLabel =
    data.collectionMode === "monthly"
      ? "Monthly Renewal"
      : data.collectionMode === "package"
        ? "Package Renewal"
        : "Due Payment";
...
<title>Payment Invoice \u2013 ${invoiceNo}</title>
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .payment-table .old-due td:last-child { color: #dc2626; }
  .payment-table .cycle td:last-child { color: #6c5ce7; }
...
<div class="info-grid">
  ...
  <div class="info-row"><span class="label">Payment Type</span><span class="value">${modeLabel}</span></div>
</div>

<div class="section-title">Payment Summary</div>
<table class="payment-table">
  ${data.oldDueAmount > 0 ? `<tr class="old-due"><td>Old Dues${data.oldDueDescription ? ` (${data.oldDueDescription})` : ""}</td><td>+${fmtMoney(data.oldDueAmount)}</td></tr>` : ""}
  ${data.cycleCharge > 0 ? `<tr class="cycle"><td>${data.cycleDescription || "Cycle Charge"}</td><td>+${fmtMoney(data.cycleCharge)}</td></tr>` : ""}
