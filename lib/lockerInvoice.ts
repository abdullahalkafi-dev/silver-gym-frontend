// lib/lockerInvoice.ts
// Professional locker payment invoice generator
...
export interface LockerInvoiceData {
  lockerNumber: number;
  memberName: string;
  invoiceNo: string;
  months: number;
...
export function openLockerInvoice(data: LockerInvoiceData): void {
  ...
  const statusText = isPaid ? "PAID" : data.paidAmount > 0 ? "PARTIAL" : "DUE";
...
<title>Locker Payment Invoice \u2013 ${invoiceNo}</title>
<style>
...
<div class="header">
  ...
  <div class="invoice-meta">
    <div class="title">Locker Payment</div>
    <div class="inv-number">${invoiceNo}</div>
    <div class="date">${dateStr} ${timeStr}</div>
  </div>
</div>

<div class="section-title">Locker Details</div>
<div class="info-grid">
  <div class="info-row"><span class="label">Locker Number</span><span class="value">#${data.lockerNumber}</span></div>
  <div class="info-row"><span class="label">Member Name</span><span class="value">${safe(data.memberName)}</span></div>
  <div class="info-row"><span class="label">Period</span><span class="value">${fmtDate(data.periodStart)} - ${fmtDate(data.periodEnd)}</span></div>
  <div class="info-row"><span class="label">Months</span><span class="value">${data.months}</span></div>
</div>

<div class="section-title">Payment Summary</div>
<table class="payment-table">
  <tr><td>Amount per Month</td><td>${fmtMoney(data.amountPerMonth)}</td></tr>
